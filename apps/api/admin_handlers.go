...
func (app *App) AdminTopup(w http.ResponseWriter, r *http.Request) {
	_, ok := getUserID(r) // caller must be admin; RequireAdmin middleware already checked
	...
	if err := app.DB.QueryRow(r.Context(), `SELECT id FROM wallets WHERE user_id=$1`, body.UserID).Scan(&userWalletID); err != nil {
		httpError(w, http.StatusBadRequest, "target_wallet_not_found"); return
	}
	...
	err = tx.QueryRow(r.Context(), `
		...
		RETURNING id
	`, idem, body.Amount).Scan(&txID)
	if err != nil {
		httpError(w, http.StatusInternalServerError, "insert_tx_error"); return
	}
	...
	`, txID, systemWalletID, body.Amount, userWalletID); err != nil {
		httpError(w, http.StatusInternalServerError, "insert_ledger_error"); return
	}
